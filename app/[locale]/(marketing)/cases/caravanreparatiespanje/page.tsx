...
export async function generateMetadata({
  ...
  return pageMetadata(locale, "caseCaravanreparatie");
}
...
/**
 * Detail-page voor de reparatie-portaal-case. Server-component, leest
 * uit `messages/*.json` `casesPage.detail.caravanreparatie.*`. Zelfde
 * structuur als de caravanverhuur-detail-page; videoUrl is hier null
 * dus we tonen een microlink-screenshot van de live site.
 */
export default async function CaravanreparatieDetailPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("casesPage.detail.caravanreparatie");
  const tRaw = await getTranslations();
  const metrics = tRaw.raw("casesPage.detail.caravanreparatie.metrics") as Metric[];
  const problemItems = tRaw.raw("casesPage.detail.caravanreparatie.problem.items") as ProblemItem[];
  const scopeItems = tRaw.raw("casesPage.detail.caravanreparatie.scope.items") as string[];
  const stackItems = tRaw.raw("casesPage.detail.caravanreparatie.stack.items") as string[];
  const quote = tRaw.raw("casesPage.detail.caravanreparatie.quote") as Quote;
  const liveUrl = "https://caravanreparatiespanje.vercel.app";

  // BuildTimeline strings hergebruiken van /diensten — zelfde 4-weken-rib.
  ...
  const detailPath =
    locale === "es" ? "/es/cases/caravanreparatiespanje" : "/cases/caravanreparatiespanje";
    ...
    <main className="dotted-bg flex flex-1 flex-col">
      <JsonLd
        data={breadcrumbLd([
          ...
          { name: "Caravanreparatiespanje", url: siteUrl(detailPath) },
        ])}
      />
      {/* HERO */}
      ...
      <section className="py-section border-t border-(--color-border) px-6">
        <RevealOnScroll className="mx-auto max-w-3xl space-y-5 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            ...
            >
              caravanreparatiespanje.vercel.app <ExternalLink className="h-3.5 w-3.5" />
            </a>
          </div>
        </RevealOnScroll>
      </section>
