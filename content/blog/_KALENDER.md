...
1. **reparatie-portaal-voor-werkplaatsen** — "reparatie portaal", "werkbon software werkplaats".
   iPad-flow, papierloos, status-tracking. Verwijst naar de Caravanreparatiespanje-case.
2. **admin-systeem-op-maat-vs-excel** — "admin systeem op maat", "bedrijf draait op excel".
   Wanneer is de stap weg van Excel logisch, wat krijg je ervoor.
3. **mkb-maatwerk-software-wanneer-wel-niet** — "maatwerk software mkb", "custom software wanneer".
   Eerlijke afbakening: wanneer SaaS, wanneer maatwerk.
4. **boekhouding-koppelen-aan-verhuursysteem** — "holded koppelen", "e-boekhouden integratie verhuur".
   Hoe een koppeling werkt, NL+ES dubbele boekhouding.
5. **verhuursoftware-begrippen-woordenlijst** — glossary-post: "channel manager", "dubbele boeking",
   "klantportaal", "multi-currency", "borg-automatisering". Long-tail traffic, interne links naar
   alle relevante pagina's.
6. **seo-voor-lokale-bedrijven-spanje** (ES) — uitbreiding van seo-local-espana, dieper.
7. **avantio-alternativa-personalizada (ES)** afmaken naar 1000+ woorden (nu nog skeleton).
