...
export function AvailabilityPill({ href, children }: { href: Href; children: React.ReactNode }) {
  ...
  return (
    ...
    >
      <Link
        href={href}
        className="shadow-card inline-flex items-center gap-2 rounded-full border border-(--color-border) bg-(--color-surface) px-3.5 py-1.5 text-[13px] text-(--color-muted) transition-colors hover:text-(--color-text) focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--color-accent)"
      >
        <span
