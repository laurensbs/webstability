...
export function ServiceCard({
  ...
  return (
    <motion.article
      ...
      className="group shadow-card rounded-panel relative flex h-full flex-col overflow-hidden border border-(--color-bg)/15 bg-(--color-text) p-7 text-(--color-bg) transition-all duration-300 hover:shadow-[0_2px_4px_rgba(31,27,22,0.08),0_24px_48px_-12px_rgba(107,30,44,0.32)] sm:p-9"
    >
      ...
      {/* Icon-tegel */}
      <div className="group-hover:shadow-glow rounded-card relative mb-6 grid h-12 w-12 place-items-center bg-(--color-accent)/15 text-(--color-accent) transition-all duration-300 group-hover:rotate-[-6deg] group-hover:bg-(--color-accent) group-hover:text-white">
